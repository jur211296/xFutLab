...
with st.expander("🔍 Filtros de jugadores a analizar", expanded=True):
    ...
    with col7:
        ...
        if callable(_seg):
            try:
                sel_vis = _seg("Posición general", opciones_vis, selection_mode="multi", default=default_vis, key="pos_gen_seg_pers")
            except Exception:
                sel_vis = _seg("Posición general", opciones_vis, default=default_vis, key="pos_gen_seg_pers")
        else:
            sel_vis = st.multiselect("Posición general", opciones_vis, default=default_vis, key="pos_gen_seg_pers")
...
# --- Resaltado global (catálogo sin filtros): equipo y jugador ---
try:
    equipos_catalogo, jugadores_catalogo_df, df_all_glob = obtener_catalogos_globales(DATA_PARQUET)
except Exception:
    equipos_catalogo, jugadores_catalogo_df, df_all_glob = [], pd.DataFrame(columns=["ID", "label", "Equipo_data_full"]), pd.DataFrame()

# Mapa de label -> set de IDs (por si hay duplicados de etiqueta)
_label_to_ids = {}
for lbl, grp in jugadores_catalogo_df.groupby("label"):
    _label_to_ids[lbl] = set(grp["ID"].tolist())

st.markdown("### 🎯 Resaltado por equipo/jugador")
col_rs1, col_rs2 = st.columns(2)
with col_rs1:
    equipos_resaltar = st.multiselect(
        "Equipos",
        options=equipos_catalogo,
        default=[],
        help="Si seleccionas equipos, se añadirán al gráfico aunque no cumplan los filtros previos. Si además eliges jugadores, se colorearán sólo esos jugadores; el resto queda en gris."
    )
with col_rs2:
    if equipos_resaltar:
        jugadores_opciones = (
            jugadores_catalogo_df[jugadores_catalogo_df["Equipo_data_full"].isin(equipos_resaltar)]["label"].dropna().unique().tolist()
        )
    else:
        jugadores_opciones = jugadores_catalogo_df["label"].dropna().unique().tolist()
    jugadores_resaltar = st.multiselect(
        "Jugadores",
        options=sorted(jugadores_opciones, key=_sort_key_az),
        default=[],
        help="Opcional. Si seleccionas jugadores, solo ellos se colorearán (usando el color primario de su equipo). El resto queda en gris."
    )

# --- Añadir al gráfico equipos/jugadores seleccionados aunque no pasen los filtros previos ---
try:
    df_union = df_agg.copy()
    ids_extra = set()
    # Prioridad: si hay jugadores seleccionados, sólo ellos
    if len(jugadores_resaltar) > 0:
        for lbl in jugadores_resaltar:
            ids_extra |= _label_to_ids.get(lbl, set())
    elif len(equipos_resaltar) > 0:
        # Si no hay jugadores, añadir todos los de los equipos seleccionados
        if df_all_glob is not None and not df_all_glob.empty:
            # Asegurar Equipo_data_full
            if "Equipo_data_full" not in df_all_glob.columns:
                if {"Equipo_data", "Pais_diminutivo"}.issubset(df_all_glob.columns):
                    df_all_glob = df_all_glob.copy()
                    df_all_glob["Equipo_data_full"] = df_all_glob["Equipo_data"].astype(str) + " " + df_all_glob["Pais_diminutivo"].astype(str)
                elif "Equipo_data" in df_all_glob.columns:
                    df_all_glob = df_all_glob.copy()
                    df_all_glob["Equipo_data_full"] = df_all_glob["Equipo_data"].astype(str)
            ids_extra = set(
                df_all_glob.loc[df_all_glob["Equipo_data_full"].astype(str).isin(set(equipos_resaltar)), "ID"].dropna().unique().tolist()
            )
    if ids_extra:
        df_extra_raw = df_all_glob.loc[df_all_glob["ID"].isin(ids_extra)].copy()
        if not df_extra_raw.empty:
            # Agregación por ID como en el flujo principal
            if "Torneo" in df_extra_raw.columns and df_extra_raw["Torneo"].nunique() > 1:
                columnas_sumables = [c for c in df_extra_raw.columns if is_numeric_dtype(df_extra_raw[c]) and c not in ["Edad"]]
                agg_spec = {col: "sum" for col in columnas_sumables}
                opcionales_first = [
                    "Nombre_transfermarket", "ID_Equipo", "logo_equipo", "Equipo_data",
                    "Posicion_general", "Posicion_detallada", "Pais",
                ...
                for col in opcionales_first:
                    if col in df_extra_raw.columns:
                        agg_spec[col] = "first"
                if "Torneo" in df_extra_raw.columns:
                    agg_spec["Torneo"] = lambda x: list(set(x.dropna()))
                df_extra_agg = df_extra_raw.groupby("ID", as_index=False).agg(agg_spec)
            else:
                df_extra_agg = df_extra_raw.copy()
            # Aplicar métricas personalizadas para alinear con df_agg
            df_extra_proc, _, _, _, _ = aplicar_metricas_personalizadas(df_extra_agg, df_metricas)
            # Unir y eliminar duplicados por ID
            df_union = pd.concat([df_union, df_extra_proc], ignore_index=True)
            if "ID" in df_union.columns:
                df_union = df_union.drop_duplicates(subset=["ID"], keep="first")
    # Usar df_union para visualización en adelante
    df_agg = df_union
except Exception:
    pass
...
if len(df_agg) > 0:
    ...
    if reduce_detail:
        ...
        # Si el zoom deja la muestra vacía, detener de forma segura
        if df_viz.empty:
            st.warning("No hay jugadores dentro del rango seleccionado. Ajusta los sliders de zoom.")
            st.stop()
        apply_zoom = True
    else:
    ...
    # Recalcular percentiles sobre la muestra actual de forma segura
    ...
    p25_x, p75_x = _safe_pct(df_viz[x_metric])
    p25_y, p75_y = _safe_pct(df_viz[y_metric])
    ...
    with st.spinner("Cargando gráfico simple..."):
        ...
        # --- Aplicar resaltado (gris para no seleccionados) ---
        apply_highlight = False
        highlight_mask = pd.Series(False, index=df_viz.index)

        # Caso 1: si hay jugadores seleccionados, priorizar jugadores
        if len(jugadores_resaltar) > 0:
            ids_target = set()
            for lbl in jugadores_resaltar:
                ids_target |= _label_to_ids.get(lbl, set())
            if "ID" in df_viz.columns and len(ids_target) > 0:
                highlight_mask = df_viz["ID"].isin(ids_target)
                apply_highlight = True
            else:
                # Fallback por nombre si no hay ID (menos robusto)
                if "ID_Display" in df_viz.columns:
                    highlight_mask = df_viz["ID_Display"].astype(str).isin(jugadores_resaltar)
                elif "Nombre_transfermarket" in df_viz.columns:
                    highlight_mask = df_viz["Nombre_transfermarket"].astype(str).isin(jugadores_resaltar)
                else:
                    highlight_mask = pd.Series(False, index=df_viz.index)
                apply_highlight = True
        # Caso 2: si no hay jugadores pero sí equipos seleccionados, resaltar todos los de esos equipos
        elif len(equipos_resaltar) > 0:
            team_col = "Equipo_data_full" if "Equipo_data_full" in df_viz.columns else ("Equipo_data" if "Equipo_data" in df_viz.columns else None)
            if team_col is not None:
                highlight_mask = df_viz[team_col].astype(str).isin(set(equipos_resaltar))
                apply_highlight = True

        if apply_highlight:
            color_values = [c if m else "#C0C0C0" for c, m in zip(color_values, highlight_mask.tolist())]
        ...
        # --- Forzar arrays JSON-safe (sin NaN/Inf) para Plotly ---
        def _json_safe_floats(seq):
            out = []
            for v in list(seq):
                try:
                    fv = float(v)
        ...
        # Convertir x/y a listas puras de Python con None en vez de NaN/Inf
        x_vals = _json_safe_floats(x_plot.tolist())
        y_vals = _json_safe_floats(y_plot.tolist())

        # Asegurar colores hex válidos en todo el vector
        color_values = [(_clean_hex_color(c) or "#C0C0C0") for c in color_values]
        ...
        if show_percentiles and all(v is not None and np.isfinite(v) for v in [p25_x, p75_x, p25_y, p75_y]):
