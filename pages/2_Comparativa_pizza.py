...
if st.session_state['1v1_step'] == 3:
    ...
    titulo_1 = f"{jugador_1['Nombre_transfermarket']} {jugador_1['Temporada']} ({jugador_1['Equipo_data']})"
    titulo_2 = f"{jugador_2['Nombre_transfermarket']} {jugador_2['Temporada']} ({jugador_2['Equipo_data']})"
    with st.spinner("Generando gráfico Pizza..."):
        fig1 = crear_pizza_chart(
            jugador_1,
            metricas=seleccionadas,
            df_contexto=df_comparacion,
            metricas_invertir=metricas_invertir,
        ...
        # --- Títulos, subtítulos (2 líneas) y pie estilo scatter ---
        # Colores dependientes del tema ya calculados arriba (font_color)
        for _fig, _titulo in ((fig1, titulo_1), (fig2, titulo_2)):
            try:
                # 1) Eliminar cualquier título/subtítulo que `crear_pizza_chart` haya dejado
                #    (suptitle y fig.text en la parte superior del lienzo).
                try:
                    # suptitle nativo de Matplotlib
                    if hasattr(_fig, "_suptitle") and getattr(_fig, "_suptitle") is not None:
                        try:
                            _fig._suptitle.remove()
                        except Exception:
                            pass
                except Exception:
                    pass
                # fig-level texts en la zona alta (y >= 0.85)
                for _tx in list(getattr(_fig, "texts", [])):
                    try:
                        _x, _y = _tx.get_position()
                    except Exception:
                        _y = 1.0
                    if _y >= 0.85:
                        try:
                ...
                # 2) Añadir SOLO nuestro título en negrita y el pie
                _fig.suptitle(_titulo, fontsize=16, fontweight='bold', color=font_color, y=0.98)
                _fig.text(0.5, 0.02, "Elaborado por Jurgen Schmidt.", ha='center', va='bottom', fontsize=8, color=font_color, alpha=0.7)
            except Exception:
                pass
        col1, col2  = st.columns(2)
    ...
    # ==================== PIZZAS POR BLOQUE (2 columnas: J1 a la izquierda, J2 a la derecha) ====================
    st.markdown("---")
    st.markdown("### Pizzas por bloque")

    # Definición de bloques (mantenemos la misma agrupación sugerida)
    bloques = [
    ...
    # Títulos por jugador (mismo formato que arriba)
    titulo_base_j1 = f"{jugador_1['Nombre_transfermarket']} {jugador_1['Temporada']} ({jugador_1['Equipo_data']})"
    titulo_base_j2 = f"{jugador_2['Nombre_transfermarket']} {jugador_2['Temporada']} ({jugador_2['Equipo_data']})"

    for nombre_bloque, lista_metricas in bloques:
        # Filtrar métricas del bloque válidas para el modo y disponibles en el DF de contexto
        metricas_filtradas = [
        ...
        with col_izq:
            with st.spinner(f"Generando pizza {nombre_bloque} – {jugador_1['Nombre_transfermarket']}..."):
                fig_j1_blk = crear_pizza_chart(
                    jugador_1,
                    ...
                    considerar_dict=considerar_dict,
                    tipos_dict=tipos_dict,
                    titulo=f"{nombre_bloque} — {titulo_base_j1}",
                    subtitulo=None,
                    modo_claro=modo_claro,
                )
                st.pyplot(fig_j1_blk)
        with col_der:
            with st.spinner(f"Generando pizza {nombre_bloque} – {jugador_2['Nombre_transfermarket']}..."):
                fig_j2_blk = crear_pizza_chart(
                    jugador_2,
                    metricas=metricas_filtradas,
                    df_contexto=df_comparacion,
                    metricas_invertir=metricas_invertir,
                    considerar_dict=considerar_dict,
                    tipos_dict=tipos_dict,
                    titulo=f"{nombre_bloque} — {titulo_base_j2}",
                    subtitulo=None,
                    modo_claro=modo_claro,
                )
                st.pyplot(fig_j2_blk)