...
# Contexto: usar la muestra ya transformada con métricas
# (df_comparacion proviene de aplicar_metricas_personalizadas(df_muestra))
...
# Título y subtítulo del radar principal
titulo_grafico = f"{jugador_1['Nombre_transfermarket']} {jugador_1['Temporada']} ({jugador_1['Equipo_data']})"
posiciones_dimin = list(dict.fromkeys([diminutivos_pos.get(p, p) for p in df_comparacion.get("Posicion_detallada", pd.Series(dtype=str)).dropna().astype(str).unique().tolist()]))
...
# Radar StatsBomb principal (añadimos modo_claro)
col_izq_r, col_centro_r, col_der_r = st.columns([0.3, 0.4, 0.3])
with col_centro_r:
    fig = crear_radar_statsbomb(
        jugador=jugador_1,
        df_contexto=df_viz,
        seleccionadas=metricas_finales,
...
# Renderizar en HTML con estilo moderno y barra de progreso por percentil
tbl_css = """
<style>
...
# ---- Mini-radares por tipo de métrica ----
st.markdown("#### Comparativas por tipo de métrica")
considerar = ["/90", "Porcentaje"] if modo_90 else ["Totales", "Porcentaje"]
...
for i in range(0, len(bloques), 3):
    ...
    for (titulo, lista_metricas, color), col in zip(bloques[i:i+3], [col1, col2,col3]):
        metricas_filtradas = [
            ...
            if considerar_dict.get(m) in considerar and m in df_comparacion.columns
        ]
        ...
        with col:
            fig = crear_radar_statsbomb(
                ...
                seleccionadas=metricas_filtradas,
                metricas_invertir=metricas_invertir_ctx,
