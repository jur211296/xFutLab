...
with st.expander("🔍 Filtros de jugadores a analizar", expanded=True):
    ...
    # --- Defaults reactivos (1/3 del máximo) y reseteo por contexto ---
    ctx_hash_logos = f"{int(temporada)}|{';'.join(sorted(map(str, torneos_sel)))}|{';'.join(sorted(map(str, equipos_sel)))}"
