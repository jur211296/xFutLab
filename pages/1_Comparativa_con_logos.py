...
with st.expander("🔍 Filtros de jugadores a analizar", expanded=True):
    ...
    # --- Defaults reactivos (1/3 del país con máximo menor) y reseteo por contexto ---
    # Contexto que dispara reinicio de sliders: Temporada / País / Torneo / Equipo
    ctx_hash_logos = f"{int(temporada)}|{';'.join(sorted(map(str, paises_sel)))}|{';'.join(sorted(map(str, torneos_sel)))}|{';'.join(sorted(map(str, equipos_sel)))}"

    # Máximos actuales agregados por ID (suma entre torneos)
    ...
    # Helper para 1/3 con redondeo al múltiplo de step
    ...
    # Base 1/3 = para cada País, tomar el máximo de minutos por ID y luego el mínimo entre Países
    scope = df_temp[
    ...
    # Si cambió el contexto, reiniciar valores programáticamente
    if st.session_state.get("logos_ctx_hash_minmax") != ctx_hash_logos:
        st.session_state["logos_min_jugados_value"] = default_min_jug
        st.session_state["logos_min_m90s_value"] = default_min_m90
        st.session_state["logos_ctx_hash_minmax"] = ctx_hash_logos

    # Preinicializar para evitar warning de Streamlit (no usar value= con key en sliders)
    st.session_state.setdefault("logos_min_jugados_value", default_min_jug)
    st.session_state.setdefault("logos_min_m90s_value", default_min_m90)
    ...
    with col7:
        ...
        if callable(_seg):
            try:
                sel_vis = _seg("Posición general", opciones_vis, selection_mode="multi", default=default_vis, key="pos_gen_seg_logos")
            except Exception:
                sel_vis = _seg("Posición general", opciones_vis, default=default_vis, key="pos_gen_seg_logos")
        else:
            sel_vis = st.multiselect("Posición general", opciones_vis, default=default_vis, key="pos_gen_seg_logos")
...
# --- Filtro final aplicado sobre df_temp (corte por ID agregado y luego conservar torneos) ---
if df_temp.empty:
    st.warning("No hay datos para la temporada seleccionada.")
    st.stop()

# Muestra base por País/Torneo/Equipo (sin aplicar aún minutos/edad)
...
st.markdown("## Comparativa con logos")
st.warning("🧩 Esta visualización usa logos de equipos desde la web y puede tardar varios segundos en cargar con muestras grandes.")
...
modo_90 = st.toggle("Métricas: Totales ↔ Por 90'", value=True)
...
with st.expander("🎛️ Parámetros de visualización", expanded=True):
    ...
    index_x = 0
    index_y = 1 if len(metricas_y_candidatas) > 1 else 0
    colx, coly = st.columns(2)
    with colx:
        x_metric = st.selectbox("Métrica a representar en el eje X", metricas_x_candidatas, index=index_x)
    with coly:
        y_metric = st.selectbox("Métrica a representar en el eje Y", metricas_y_candidatas, index=index_y)
...
if len(df_agg) > 0:
    ...
    if reduce_detail:
        with st.expander("Rangos por métrica (zoom)", expanded=True):
            ...
            with col1:
                x_range = st.slider(
                    f"Rango de {x_metric}", min_x, max_x, (min_x, max_x), step=step_x,
                    help="Límite inferior y superior para la métrica del eje X."
                )
            with col2:
                y_range = st.slider(
                    f"Rango de {y_metric}", min_y, max_y, (min_y, max_y), step=step_y,
        ...
        if top_n_max <= 1:
            ...
            top_n = st.slider(
                ...
                min_slider, top_n_max, top_n_max, step=1,
    ...
    with st.spinner("Cargando gráfico simple..."):
        ...
        # Preparar logos: usar URL de 'logo_equipo' cuando exista; si no, mostrar un círculo
        if "logo_equipo" in df_viz.columns:
            logos_series = df_viz["logo_equipo"].astype(str)
            has_logo = logos_series.str.startswith(("http://", "https://"))
            logos_urls = logos_series.where(has_logo, None).tolist()
        else:
            has_logo = pd.Series([False] * len(df_viz), index=df_viz.index)
            logos_urls = [None] * len(df_viz)

        # Tamaño de logo relativo al rango de los ejes (DINÁMICO y más grande por defecto)
        span_x = (x_range[1] - x_range[0]) if apply_zoom else (max_x - min_x)
        span_y = (y_range[1] - y_range[0]) if apply_zoom else (max_y - min_y)
        span_x = span_x if span_x > 0 else 1.0
        span_y = span_y if span_y > 0 else 1.0
        n_pts = len(df_viz)
        # Interpolación: pocos puntos => logos grandes; muchos puntos => más pequeños
        breakpoints_n = np.array([1, 10, 50, 100, 200, 400])
        sizes = np.array([0.14, 0.12, 0.10, 0.085, 0.070, 0.060])  # fracción del rango por eje
        logo_frac = float(np.interp(n_pts, breakpoints_n, sizes))
        logo_sizex = span_x * logo_frac
        logo_sizey = span_y * logo_frac
        ...
        # Preparar máscaras y arrays
        mask_logo = has_logo.values if isinstance(has_logo, pd.Series) else np.array(has_logo)
        x_arr = np.array(x_plot)
        y_arr = np.array(y_plot)
        names_arr = np.where(df_viz["Mostrar_nombre"], df_viz["Nombre_transfermarket"].astype(str), "")
        names_arr = np.array([_sanitize_text(t) for t in names_arr])
        cd_np = np.array(customdata, dtype=object)

        # --- Trace 1: puntos con LOGO (marcador totalmente transparente, sin borde) ---
        if mask_logo.any():
            fig.add_trace(go.Scatter(
                x=x_arr[mask_logo],
                y=y_arr[mask_logo],
                mode="markers+text",
                text=names_arr[mask_logo].tolist(),
                textposition="top center",
                marker=dict(
                    size=30,  # grande para capturar hover; el logo irá encima
                    color="rgba(0,0,0,0)",
                    line=dict(width=0),
                    opacity=1
                ),
                customdata=cd_np[mask_logo].tolist(),
        ...
        # --- Trace 2: puntos SIN LOGO (círculo gris, tamaño 12, sin borde) ---
        if (~mask_logo).any():
            fig.add_trace(go.Scatter(
                x=x_arr[~mask_logo],
                y=y_arr[~mask_logo],
        ...
        # Superponer logos como layout.images en las coordenadas de cada jugador
        for xv, yv, url in zip(x_plot, y_plot, logos_urls):
            if url:
